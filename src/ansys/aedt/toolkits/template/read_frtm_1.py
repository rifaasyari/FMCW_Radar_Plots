    ...
    def __init__(self, filepath):
        ...
        with open(filepath, "rb") as binary_file:
            ...
            while string_to_stop_reading_header not in line_str:
                ...
                elif '@ FreqSweep ' in line_str:    # use this section to read FRMT for FMCW Radar
                    freq_sweep_line = line_str  
                    c = freq_sweep_line.split("=")
                    c = c[1].split(" ")
                    c = [i for i in c if i] 
                    self.freq_start =  float(c[1].replace("\"", ""))
                    self.freq_stop = float(c[2].replace("\"", ""))
                    self.nfreq = int(c[3].replace("\"", ""))+1
                ...
                # elif '@ FreqSweep ' in line_str: # use this section to read FRMT for Pulsed Radar
                #     freq_sweep_line = line_str  
                #     c = freq_sweep_line.split("=")
                #     c = c[1].split(" ")
                #     c = [i for i in c if i] 
